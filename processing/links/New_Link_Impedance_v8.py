...
def add_f_roadway_to_Data_Links_Rev3(mrs_links_index, f_bikeaccom_index, f_road_class_index, input_file, output_file):
    ...
    with open(input_file, newline='', encoding='utf-8-sig') as fin, \
        ...
        new_fields = ['Flag_slope','Flag_LTS','Flag_road_type','Flag_POIs', 'Flag_Scenario_LTS']
        ...
        for row in reader:
            total_rows += 1
            if (total_rows > 9999 and  total_rows % 10000 == 0):
            ...
            road_base = row.get('highway', 'missing').strip()
            class_code = f_road_class_index.get(road_base).get("value")
            ...
            #Slope
            slope_raw = row.get('slope_pct', '0.0')
            slope = 0
            if slope_raw == '':
                slope = 0.0
            ...
            #print(f"LTS {LTS_value}, VOL {volume_key} for {volume}, on a {road_class}, with a {f_bikeaccom}, at {sl_speed_limit_final}")
            
            # Additional calculation for 30km/hr speed limit change
            scenario_LTS_value = LTS_value
            if road_base == 'residential' :
                scenario_LTS_value = mrs_links_index[key]['30']
                #print(f"Adj LTS {scenario_LTS_value}")
            
            #POIs
            pois = int(row.get('Count_POI'))
            ...
            row['Flag_Scenario_LTS']=scenario_LTS_value
